...
class MafiaNetwork(Network):
    ...
    def vote(self, nodes):
        votes = {}
        for node in nodes:
            votes = Info.query.filter_by(origin_id=node.id, type='vote').order_by('creation_time')
            if len(votes) == 0:
                vote = Node.query.filter_by(property2='True').all()[0].property1
            else:
                vote = infos[-1].contents.split(': ')[1]
                if Node.query.filter_by(property1=vote).one().property2 == 'False':
                    vote = Node.query.filter_by(property2='True').all()[0].property1
            if vote in votes:
                votes[vote] += 1
            else:
