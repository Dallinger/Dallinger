...
def phase(node_id, switches, was_daytime):
    try:
        ...
        if int(elapsed_time.total_seconds()) != 0 and not daytime and (int(elapsed_time.total_seconds() - switches / 2 * daybreak_duration) % night_round_duration == 0):
        ...
        # If it's day but should be night, then call setup_nighttime()
            ...
            victim_name = Node.query_filter_by(network_id=net.id, property2='False').order_by('deathtime').all()[-1].property1
            if daytime:
