...
def ensure_constraints_file_presence(directory: str, update_existing: bool = False):
    ...
    If no ``constraints.txt`` file exists, it will be generated using ``generate_constraints``.
    If a ``constraints.txt`` file exists already, but no subsequent changes have been made to the input file
    (i.e. ``requirements.txt`` or ``pyproject.toml``), then no action will be taken.
    If a ``constraints.txt`` file exists already, but subsequent changes have been made to the input file,
    then a ``ValueError`` will be raised, unless ``update_existing`` is ``True`, in which case
    the ``constraints.txt`` file will be automatically updated.
