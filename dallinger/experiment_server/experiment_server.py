...
def launch():
    ...
    if exp.channel is not None:
        try:
            # Neither subscription is waited on. Until a participant
            # connects, the only publishes Dallinger makes on these channels
            # are the "subscribed" control events these two calls announce
