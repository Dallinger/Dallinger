...
def _make_hash(input_path: Path, python_version: str) -> str:
    with open(input_path, "rb") as f:
        file_bytes = f.read()
    combined = file_bytes + python_version.encode("utf-8")
...
def _constraints_up_to_date(input_path: Path) -> bool:
    ...
    return _make_hash(input_path, python_version) in constraints_path.read_text()
...
def _pip_compile(in_file, out_file, constraints: Optional[list] = None):
    ...
    md5sum = _make_hash(in_file, requested_python_version)
    compile_info = f"dallinger constraints generate\n#\n# Compiled from a {Path(in_file).name} file with md5sum: {md5sum}"

    if use_uv:
        logger.info("Calling `uv pip-compile`...")
        compile_info += f"\n# Python {requested_python_version}"
