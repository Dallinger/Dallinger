...
def test_executor_run_logs_remote_failure_details(monkeypatch, capsys):
    import importlib
    ...
    class DummyChannel:
        ...
        def recv(self, _size):
            return b"remote stdout error"

        def recv_stderr(self, _size):
            return b"remote stderr error"
    ...
    executor = docker_ssh.Executor.__new__(docker_ssh.Executor)
    executor.client = DummyClient()
    executor.app = "dlgr-deadbeef"
    executor.print_docker_compose_logs = mock.Mock()

    with pytest.raises(docker_ssh.ExecuteException) as excinfo:
        executor.run("echo fail")

    output = capsys.readouterr().out
    assert "Error: exit code was not 0 (1)" in output
    assert "remote stdout error" in output
    assert "remote stderr error" in output
    executor.print_docker_compose_logs.assert_called_once()
    assert "echo fail" in str(excinfo.value)


def test_build_and_push_image_logs_registry_lookup_errors(monkeypatch, capsys):
    import importlib
    ...
    class FakeConfig:
        def get(self, key, default=None):
            values = {
                "docker_image_name": "ghcr.io/dallinger/dallinger/test-image:latest",
                "docker_image_base_name": "ghcr.io/dallinger/dallinger/base",
            }
    ...
    class FakeImages:
        def get_registry_data(self, _image_name):
    ...
    monkeypatch.setattr(docker_ssh, "get_config", lambda load=True: FakeConfig())
    monkeypatch.setattr(
        docker_ssh, "setup_experiment", lambda *args, **kwargs: ("uid", "/tmp/mock")
    )
    monkeypatch.setattr(
        "dallinger.docker.tools.build_image",
        lambda *args, **kwargs: "ghcr.io/dallinger/dallinger/test-image:built",
    )
    monkeypatch.setattr(
        docker_cmd.push,
        "callback",
        lambda **kwargs: "ghcr.io/dallinger/dallinger/test-image:pushed",
    )
    monkeypatch.setattr(docker, "from_env", lambda use_ssh_client=True: fake_client)

    @docker_ssh.build_and_push_image
    def wrapped(**kwargs):
        return kwargs["image_name"]

    result = wrapped(
        server="localhost",
        app_name="dlgr-deadbeef",
        archive_path=None,
        config_options=(),
        dns_host="nip.io",
        update=False,
    ...
    output = capsys.readouterr().out
    assert "Error checking remote image: registry unavailable" in output
    assert result == "ghcr.io/dallinger/dallinger/test-image:pushed"
    assert fake_client.closed


def test_build_and_push_image_propagates_local_build_errors(monkeypatch):
    import importlib
    ...
    monkeypatch.setattr(
        "dallinger.docker.tools.build_image",
        mock.Mock(side_effect=RuntimeError("local build failed")),
    )
    monkeypatch.setattr(
        docker, "from_env", lambda use_ssh_client=True: FakeDockerClient()
    )
    ...
    with pytest.raises(RuntimeError, match="local build failed"):
        wrapped(
            server="localhost",
            app_name="dlgr-beadfeed",
            archive_path=None,
            config_options=(),
            dns_host="nip.io",
            update=False,
