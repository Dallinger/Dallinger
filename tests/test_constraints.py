import os
import platform
import re
from pathlib import Path
from unittest.mock import patch

import pytest
from click.testing import CliRunner

from dallinger.command_line import dallinger as dallinger_cli
from dallinger.constraints import (
    _compile_with_uv,
    _constraints_autogenerated,
    _get_constraints_signature,
    _get_explicit_dallinger_github_requirement,
    _get_explicit_dallinger_numbered_release,
    check_constraints,
    ensure_constraints_file_presence,
    generate_constraints,
    working_directory,
)
from dallinger.utils import check_output


@pytest.fixture
def python_version_file(tempdir):
    with open(Path(tempdir) / ".python-version", "w") as f:
        f.write(platform.python_version())


@pytest.mark.slow
class TestGenerateConstraints:
    def test_constraints_dallinger_release(self, in_tempdir, python_version_file):
        with open("requirements.txt", "w") as f:
            f.write("dallinger==11.4.0")
        generate_constraints()
        assert Path("constraints.txt").exists()
        assert _constraints_autogenerated()

        # see https://raw.githubusercontent.com/Dallinger/Dallinger/v11.4.0/dev-requirements.txt
        # # to verify that flask==3.1.1 is indeed the correct version
        assert "flask==3.1.1" in Path("constraints.txt").read_text()

        # Verify that the requested Python version is included in constraints.txt.
        content = Path("constraints.txt").read_text()
        python_version_pattern = r"Python \d+\.\d+(\.\d+)?"
        assert re.search(python_version_pattern, content), (
            "constraints.txt missing Python version information"
        )

    def test_constraints_dallinger_commit(self, in_tempdir, python_version_file):
        with open("requirements.txt", "w") as f:
            f.write(
                "dallinger@git+https://github.com/Dallinger/Dallinger.git@e18ce87985dc9cef64fe4ebcddfc1027f41c3398#egg=dallinger"
            )
        generate_constraints()

        # see https://raw.githubusercontent.com/Dallinger/Dallinger/e18ce87985dc9cef64fe4ebcddfc1027f41c3398/dev-requirements.txt
        # to verify that 3.1.1 is indeed the correct version
        assert "flask==3.1.1" in Path("constraints.txt").read_text()

    def test_constraints_psynet(self, in_tempdir, python_version_file):
        with open("requirements.txt", "w") as f:
            f.write("psynet==12.0.0")
        generate_constraints()
        assert Path("constraints.txt").exists()
        dallinger_version = _get_explicit_dallinger_numbered_release(
            Path("constraints.txt")
        )
        major, minor, patch = dallinger_version.split(".")

        # To verify the correct version see https://gitlab.com/PsyNetDev/PsyNet/-/blob/v12.0.0/pyproject.toml?ref_type=tags
        assert major == "11"
        assert int(minor) >= 1
        if int(minor) == 1:
            assert int(patch) >= 1

    def test_caching(self, tempdir, in_tempdir, python_version_file):
        requirements_path = Path("requirements.txt")
        requirements_path.write_text("dallinger==11.4.0\n")
        constraints_path = Path("constraints.txt")

        ensure_constraints_file_presence(tempdir)

        assert constraints_path.exists()
        assert "dallinger==11.4.0" in constraints_path.read_text()

        original_mtime = os.path.getmtime(constraints_path)

        ensure_constraints_file_presence(tempdir)

        # No change to requirements.txt so no change to constraints.txt
        assert os.path.getmtime(constraints_path) == original_mtime

        with open(requirements_path, "a") as f:
            f.write("scipy")

        # Change to requirements.txt so constraints.txt should be updated
        original_mtime = os.path.getmtime(constraints_path)
        ensure_constraints_file_presence(tempdir)
        assert os.path.getmtime(constraints_path) > original_mtime

    def test_constraints_written_manually(
        self, in_tempdir, tempdir, python_version_file
    ):
        with open("constraints.txt", "w") as f:
            f.write("dallinger==11.4.0")

        original_mtime = os.path.getmtime("constraints.txt")
        ensure_constraints_file_presence(tempdir)

        # Manually written constraints.txt files should not be updated
        assert os.path.getmtime(Path("constraints.txt")) == original_mtime

    def test_check_constraints(self, in_tempdir, tempdir, python_version_file):
        with open("requirements.txt", "w") as f:
            f.write("dallinger==11.4.0")
        with pytest.raises(FileNotFoundError):
            check_constraints()
        generate_constraints()
        check_constraints()


@pytest.mark.slow
def test_constraints_script_works_independently(tempdir, python_version_file):
    """Test that constraints.py runs when used as a self-contained script."""
    script_path = Path("dallinger/constraints.py").resolve()
    with working_directory(tempdir):
        check_output(["uv", "run", str(script_path), "generate"])
        check_output(["uv", "run", str(script_path), "check"])
        assert Path("constraints.txt").exists()
        check_output(["uv", "run", str(script_path), "ensure"])
        assert Path("constraints.txt").exists()


@pytest.mark.slow
def test_legacy_interface(in_tempdir, python_version_file):
    """Test that the legacy interface works."""
    check_output(["dallinger", "generate-constraints"])
    assert Path("constraints.txt").exists()


@pytest.mark.slow
def test_impossible_resolution(in_tempdir, python_version_file):
    with open("requirements.txt", "w") as f:
        f.write("dallinger==10.0.0\n")
        f.write("dallinger==10.0.1\n")
    with pytest.raises(
        RuntimeError,
        match="An error occurred when compiling the constraints.txt file. See if you can adjust",
    ):
        generate_constraints()


def test_constraints_run_from_wrong_directory(tempdir, python_version_file):
    with open(Path(tempdir) / "requirements.txt", "w") as f:
        f.write("dallinger==11.4.0\n")
    with pytest.raises(
        ValueError,
        match="not in the current working directory",
    ):
        _compile_with_uv(
            Path(tempdir) / "requirements.txt", Path(tempdir) / "constraints.txt"
        )

    with working_directory(tempdir):
        _compile_with_uv(Path("requirements.txt"), Path("constraints.txt"))


def test_get_explicit_dallinger_github_requirement(in_tempdir):
    commit_hash = "2a4290c110e72733f07bae2a26b4ff21bc680bb9"

    # TOML with double quotes
    with open("pyproject.toml", "w") as f:
        f.write("[project]\n")
        f.write("dependencies = [\n")
        f.write(
            f'    "dallinger[docker] @ git+https://github.com/Dallinger/Dallinger.git@{commit_hash}",\n'
        )
        f.write("]\n")

    result = _get_explicit_dallinger_github_requirement(Path("pyproject.toml"))
    assert result == commit_hash, f"Expected {commit_hash}, got {result}"

    # TOML with single quotes
    with open("pyproject.toml", "w") as f:
        f.write("[project]\n")
        f.write("dependencies = [\n")
        f.write(
            f"    'dallinger[docker] @ git+https://github.com/Dallinger/Dallinger.git@{commit_hash}',\n"
        )
        f.write("]\n")

    result = _get_explicit_dallinger_github_requirement(Path("pyproject.toml"))
    assert result == commit_hash, f"Expected {commit_hash}, got {result}"

    # requirements.txt (no quotes)
    with open("requirements.txt", "w") as f:
        f.write(
            f"dallinger@git+https://github.com/Dallinger/Dallinger.git@{commit_hash}#egg=dallinger\n"
        )

    result = _get_explicit_dallinger_github_requirement(Path("requirements.txt"))
    assert result == commit_hash, f"Expected {commit_hash}, got {result}"


def test_extras_influence_signature(in_tempdir):
    """Extras change the constraints signature (md5), so different extras produce different constraints."""
    Path("pyproject.toml").write_text(
        "[project]\nname = 'test'\ndependencies = ['dallinger==11.4.0']\n"
        "[project.optional-dependencies]\ndocker = ['docker']\ntest = ['pytest']\n"
    )
    Path(".python-version").write_text("3.12.0")
    sig_none = _get_constraints_signature(Path("pyproject.toml"), extras=[])
    sig_docker = _get_constraints_signature(Path("pyproject.toml"), extras=["docker"])
    sig_test = _get_constraints_signature(Path("pyproject.toml"), extras=["test"])
    sig_both = _get_constraints_signature(
        Path("pyproject.toml"), extras=["docker", "test"]
    )
    assert sig_none != sig_docker
    assert sig_docker != sig_test
    assert sig_both != sig_docker
    assert sig_both != sig_test
    # Order should not matter
    assert (
        _get_constraints_signature(Path("pyproject.toml"), extras=["test", "docker"])
        == sig_both
    )


def test_generate_upgrades_dallinger_from_existing_lock(
    in_tempdir, python_version_file
):
    """Regeneration must not keep a stale dallinger pin from constraints.txt."""
    Path("requirements.txt").write_text("dallinger==11.4.0\n")
    Path("constraints.txt").write_text("dallinger==11.4.0\n")

    def mock_side_effect(cmd, **kwargs):
        out_idx = cmd.index("--output-file") + 1
        Path(cmd[out_idx]).write_text(
            "# This file was autogenerated\ndallinger==11.4.0\n"
        )
        return b""

    with patch("dallinger.constraints.subprocess.check_output") as mock_check:
        mock_check.side_effect = mock_side_effect
        with patch("dallinger.constraints._test_dallinger_dev_requirements_path"):
            generate_constraints()

    cmd = mock_check.call_args[0][0]
    assert "--upgrade-package" in cmd
    assert cmd[cmd.index("--upgrade-package") + 1] == "dallinger"


def test_generate_upgrades_implied_dallinger_on_final_compile(
    in_tempdir, python_version_file
):
    """Implied ranges must upgrade Dallinger on the constrained compile only."""
    Path("requirements.txt").write_text("psynet==13.3.0\n")
    Path("constraints.txt").write_text("dallinger==12.2.0\n")

    def mock_side_effect(cmd, **kwargs):
        out_idx = cmd.index("--output-file") + 1
        if "--constraints" in cmd:
            Path(cmd[out_idx]).write_text(
                "# This file was autogenerated\ndallinger==12.2.1\n"
            )
        else:
            Path(cmd[out_idx]).write_text("dallinger==12.2.1\n")
        return b""

    with patch("dallinger.constraints.subprocess.check_output") as mock_check:
        mock_check.side_effect = mock_side_effect
        with patch("dallinger.constraints._test_dallinger_dev_requirements_path"):
            generate_constraints()

    implied_cmds = [
        call.args[0]
        for call in mock_check.call_args_list
        if "--constraints" not in call.args[0]
    ]
    final_cmds = [
        call.args[0]
        for call in mock_check.call_args_list
        if "--constraints" in call.args[0]
    ]
    assert implied_cmds
    assert final_cmds
    assert "--upgrade-package" not in implied_cmds[0]
    assert "--upgrade-package" in final_cmds[0]
    assert final_cmds[0][final_cmds[0].index("--upgrade-package") + 1] == "dallinger"


@pytest.mark.slow
def test_extras_passed_to_resolver(in_tempdir, python_version_file):
    """Resolver (uv pip compile) receives --extra when extras are provided."""
    Path("pyproject.toml").write_text(
        "[project]\nname = 'test'\ndependencies = ['dallinger==11.4.0']\n"
        "[project.optional-dependencies]\ndocker = ['docker']\n"
    )

    def mock_side_effect(cmd, **kwargs):
        out_idx = cmd.index("--output-file") + 1
        Path(cmd[out_idx]).write_text("# autogenerated\nflask==3.1.1\n")
        return b""

    with patch("dallinger.constraints.subprocess.check_output") as mock_check:
        mock_check.side_effect = mock_side_effect
        generate_constraints(extras=["docker"])
        call_args = mock_check.call_args[0][0]
        assert "--extra" in call_args
        extra_idx = call_args.index("--extra")
        assert call_args[extra_idx + 1] == "docker"


def test_extras_require_pyproject_toml(in_tempdir, python_version_file):
    """Using extras with only requirements.txt raises a clear error."""
    Path("requirements.txt").write_text("dallinger==11.4.0\n")
    with pytest.raises(
        ValueError,
        match="Extras require pyproject.toml. Only requirements.txt was found",
    ):
        generate_constraints(extras=["docker"])


@pytest.mark.slow
def test_extras_prefer_pyproject_when_both_exist(in_tempdir, python_version_file):
    """When extras given and both pyproject.toml and requirements.txt exist, use pyproject.toml."""
    Path("requirements.txt").write_text("dallinger==11.4.0\n")
    Path("pyproject.toml").write_text(
        "[project]\nname = 'test'\ndependencies = ['dallinger==11.4.0']\n"
        "[project.optional-dependencies]\ndocker = ['docker']\n"
    )

    def mock_side_effect(cmd, **kwargs):
        out_idx = cmd.index("--output-file") + 1
        Path(cmd[out_idx]).write_text("# autogenerated\nflask==3.1.1\n")
        return b""

    with patch("dallinger.constraints.subprocess.check_output") as mock_check:
        mock_check.side_effect = mock_side_effect
        generate_constraints(extras=["docker"])
        call_args = mock_check.call_args[0][0]
        # Input file should be pyproject.toml (path may be absolute)
        assert any("pyproject.toml" in str(a) for a in call_args)


@pytest.mark.slow
def test_dallinger_constraints_allows_extras_after_subcommand(
    in_tempdir, python_version_file
):
    Path("pyproject.toml").write_text(
        "[project]\nname = 'test'\ndependencies = ['dallinger==11.4.0']\n"
        "[project.optional-dependencies]\ndev = ['pytest']\n"
    )

    def mock_side_effect(cmd, **kwargs):
        out_idx = cmd.index("--output-file") + 1
        Path(cmd[out_idx]).write_text("# autogenerated\nflask==3.1.1\n")
        return b""

    runner = CliRunner()
    with patch("dallinger.constraints.subprocess.check_output") as mock_check:
        mock_check.side_effect = mock_side_effect
        result = runner.invoke(dallinger_cli, ["constraints", "generate", "--e", "dev"])

    assert result.exit_code == 0, result.output
    call_args = mock_check.call_args[0][0]
    assert "--extra" in call_args


def test_ensure_constraints_file_presence_compiles_in_target_directory(
    tmp_path, monkeypatch
):
    experiment = tmp_path / "experiment"
    experiment.mkdir()
    (experiment / "requirements.txt").write_text("dallinger==11.4.0\n")
    monkeypatch.chdir(tmp_path)

    # An up-to-date constraints file in the caller cwd must not short-circuit
    # compilation for a different target directory.
    python_version = platform.python_version()
    (tmp_path / ".python-version").write_text(python_version)
    caller_requirements = tmp_path / "requirements.txt"
    caller_requirements.write_text("flask\n")
    caller_constraints = (
        "# This file is autogenerated\n"
        f"Python {python_version}\n"
        f"{_get_constraints_signature(caller_requirements)}\n"
    )
    (tmp_path / "constraints.txt").write_text(caller_constraints)

    compiled_in = []

    def fake_generate(extras=None):
        compiled_in.append(Path.cwd())
        Path("constraints.txt").write_text("# generated\n")

    with patch("dallinger.constraints.generate_constraints", side_effect=fake_generate):
        ensure_constraints_file_presence(experiment)

    assert compiled_in == [experiment.resolve()]
    assert (experiment / "constraints.txt").read_text() == "# generated\n"
    assert (tmp_path / "constraints.txt").read_text() == caller_constraints
    assert (experiment / "requirements.txt").read_text() == "dallinger==11.4.0\n"


def test_dallinger_constraints_unknown_extra_is_click_error(
    in_tempdir, python_version_file
):
    Path("pyproject.toml").write_text(
        "[project]\nname = 'test'\ndependencies = ['dallinger==11.4.0']\n"
        "[project.optional-dependencies]\ndev = ['pytest']\n"
    )

    runner = CliRunner()
    result = runner.invoke(dallinger_cli, ["constraints", "generate", "--e", "sphinx"])

    assert result.exit_code != 0
    assert "Unknown extra" in result.output
    assert "Traceback" not in result.output
