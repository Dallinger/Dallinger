    ...
    [
        ...
        pytest.param(
            ...
            marks=pytest.mark.skip(
                reason=(
                    "Legacy pip-compile fallback can hang on real dependency "
                    "resolution for old Dallinger/PsyNet releases under newer "
                    "Python versions; fallback behavior is covered by targeted "
                    "tests below."
                )
            ),
        ),
...
class TestGenerateConstraints:
    ...
    def mock_uv_available(self, monkeypatch, uv_available_value):
        # These slow tests exercise real dependency resolution with uv. The legacy
        # pip-compile fallback is covered by narrower tests that do not resolve
        # old release dependency graphs end-to-end.
        monkeypatch.setattr(
...
def test_check_constraints_wrong_python_version(in_tempdir):
    ...
    with pytest.raises(
        ...
        _check_pip_compile_output(Path("constraints.txt"))
    ...
    with patch("dallinger.constraints.subprocess.check_output") as mock_check:
        ...
        with patch("dallinger.constraints.uv_available", return_value=True):
            result = runner.invoke(
                dallinger_cli, ["constraints", "generate", "--e", "dev"]
            )
